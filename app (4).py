...
# The data is already cleaned and relevant features engineered (including release_year and game_age),
...
if 'metacritic' in df_am12_bb20_rj43_cleaned.columns and 'rating' in df_am12_bb20_rj43_cleaned.columns and not df_am12_bb20_rj43_cleaned['metacritic'].isnull().all():
    ...
    # Ensure 'name', 'metacritic', and 'rating' columns exist before plotting
    if all(col in top_games_metacritic.columns for col in ['name', 'metacritic', 'rating']):
        ...
        st.info("Required columns for Metacritic vs Rating plot are missing.")
else:
...
if 'metacritic_category' in df_am12_bb20_rj43_cleaned.columns and 'ratings_category' in df_am12_bb20_rj43_cleaned.columns:
    ...
    # Ensure 'name', 'metacritic', and 'rating' columns exist before displaying
    if all(col in df_am12_bb20_rj43_loved_games.columns for col in ['name', 'metacritic', 'rating']):
        st.write("Games loved by both critics and audiences:")
        st.dataframe(df_am12_bb20_rj43_loved_games[['name','metacritic','rating']])
    else:
        st.info("Required columns for displaying games loved by critics and audiences are missing.")
else:
...
if 'publishers' in df_am12_bb20_rj43_cleaned.columns:
    ...
    # Ensure index and values exist before plotting
    if not top_publishers.empty:
        ...
        st.info("No publisher data available to plot.")
else:
...
if 'release_year' in df_am12_bb20_rj43_cleaned.columns:
    ...
    if not games_per_year.empty:
        ...
        st.info("No yearly game count data available to plot.")
else:
...
if 'release_year' in df_am12_bb20_rj43_cleaned.columns and 'metacritic' in df_am12_bb20_rj43_cleaned.columns and 'rating' in df_am12_bb20_rj43_cleaned.columns:
    ...
    if not average_ratings_per_year.empty:
        ...
        st.info("No average ratings data per year available to plot.")
else:
